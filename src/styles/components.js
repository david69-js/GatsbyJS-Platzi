...
export const StyledJumbo2 = styled.div`
  ...
  &:before{
    content: " ";
    background-color: red;
    background-color: ${colors.customcolor};
    z-index: -1;
    position: absolute;
    transform: rotate(-19deg);
    border-radius: 35%;
...
`
/*Custmo Purchase and button*/
export const CustomPurchase = styled.div`
  width: 100vw;
  height: calc(100vh - 4.5rem - 101px);
  background-color: colors.customcolor;
  display: flex;
...
`
export const CustmoButton = styled.button`
  cursor: pointer;
  background-color: ${({ type }) =>
    type === "outline" ? "transparent" : colors.gray};
  color: ${({ type }) => (type === "outline" ? colors.darkBlue : "white")};
  font-size: 0.9rem;
  padding: 0.6rem 1rem;
  font-weight: 500;
...
`
export const colorWear = styled.button`
  cursor: pointer;
